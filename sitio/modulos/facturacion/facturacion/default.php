...
if ($_REQUEST['anular']=="1"){
	...
	$sqlx="update tblfacturase set idactivo=3 where id=".$idpedido;
	if ($db->Execute($sqlx)) {
	 $sqly="select dspedido  from tblfacturase where id=".$idpedido;
	 //echo $sqly;
	...
	$vermax->Close();
	if($dspedido<>""){
	$sqll="update tblpagos set idestado=2 where idpedido=".$dspedido;
	$db->Execute($sqll);
	}
...
//echo $sql;
	...
	<a href="../facturacion/rep.facturas.listado.php" title="Click para ver reporte" class="botones"><p >Relacion de facturas</p></a>

	<a href="../facturacion/pedidos.primer.paso.php" title="Click para ingresar una nueva factura" class="botones"><p >Ingresar nueva factura</p></a>
	</td>
    </tr>
	</table>
	<?
