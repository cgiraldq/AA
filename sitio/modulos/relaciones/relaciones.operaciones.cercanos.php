...
*/
		...
		if($idultimox=="") $idultimox=$idcampo;

		if ($_REQUEST['paso']==1){

		$sql="delete from $tablarelaciones where idorigen=$idultimox";

		$db->Execute($sql);
			for ($i=0;$i<$contar;$i++){
				// borrar las asignaciones de prod e ingresar de nuevo

					$sql="insert into $tablarelaciones (idorigen,iddestino) values($idultimox,".$x[$i].")";
