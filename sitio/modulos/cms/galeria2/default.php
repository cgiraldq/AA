...
$titulomodulo="Configuraci&oacute;n categor&iacute;as de Galeria";
$dsm=$_REQUEST['dsm'];
$dsrutax=limpieza(strtolower($dsm));
$rutaImagen="../../../contenidos/images/paginagaleria/";
...
if ($dsm<>"" && $idpos<>"") {
		$sql="select id ";
		 ...
		 if (!$result->EOF) {
			...
			$sql="insert into $tabla (dsm,idpos,idactivo,idevento,dsruta)";
			$sql.=" values ('$dsm',$idpos,$idactivo,'$idevento',$dsrutax)";
			//echo $sql;
				$db->Execute($sql);
			...
			if (!$result->EOF)  {
				...
				*/
				...
				$mensajes=$men[0].".<br><br>$sql<br>Error:".mysql_error();
			}
		 }
		 $result->close();
}
