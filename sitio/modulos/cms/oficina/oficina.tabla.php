...
$nombrecampos="Id,Nombre,Ciudad,Posici&oacute;n,Activo";
		...
		?>
		 <tr bgcolor="<? echo $fondo?>" onMouseOut="mOut(this,'<? echo $fondo;?>');" onMouseOver="mOvr(this,'<? echo $fondo3;?>');" >
		  ...
		  <input type="text" name="dsm_[]" value="<? echo $result->fields[1]?>" size="50" class="textnegro" maxlength="100">
			 ...
			 <td align="center">
		  <select name="idciudad_[]" class="textnegro">
		 <? categorias('tblciudades',$result->fields[4])?>
		  </select>
			</td>
		  ...
		  <option value="1" <? if ($result->fields[3]==1) echo "selected";?>>SI</option><!-- activar -->
		  <option value="2" <? if ($result->fields[3]==2) echo "selected";?>>NO</option><!-- desactivar -->
		  <option value="3" <? if ($result->fields[3]==3) echo "selected";?>>Principal</option><!-- destacado en pagina de negocios -->
		  </select>
			</td>
		  ...
		  <td align="center">
		  <?
		  $rutax="oficina.ubicacion.php?idc=".$result->fields[0];
		  $trutax="Click para ubicar la oficina";
		  $mrutax="Ubicacion";
		  include($rutxx."../../incluidos_modulos/enlace.generico.php");?>
		  |
		  <?
		  $rutax="oficina.editar.php?idx=".$result->fields[0];
