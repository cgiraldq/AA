...
//echo "dsdsdsdsd";
//exit;
$dscobertura=trim($_REQUEST['dscobertura']);
$idcobertura=trim($_REQUEST['idcobertura']);
...
if ($dsnombre<>"") {
		...
		$sql="insert into tblcobertura_contacto ( ";
		$sql.="idcobertura,dsnombre,dsapellidos,dsemail,dsempresa,dscargo,dspais,dsciudad,dstelefono,dsmovil,dsdireccion";
		$sql.=",dscom,dsfecha,idactivo";
		$sql.=") values (";
		$sql.="$idcobertura,'$dsnombre','$dsapellidos',";
		$sql.="'$dsemail','$dsempresa','$dscargo','$dspais','$dsciudad','$dstelefono','$dsmovil','$dsdireccion',";
		$sql.="'$dscom','".date("d/m/Y")."',1)";		
		//echo $sql;				
		//exit;
		$db->Execute($sql);//exit();
		$redir="../../gracias.solicitar.php?idg=4";
} else { 
	$redir="../../gracias.solicitar.php?idg=4";
}		
