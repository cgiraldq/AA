...
$nom_producto=seldato("dsm","id","tblproductos",$idproducto,1);//
...
if ($dsnombre<>"") {
		...
		$sql.="idproducto,dsnombre,dsapellidos,dsemail,dsempresa,dscargo,dspais,dsciudad,dstelefono,dsmovil,dsdireccion";
		$sql.=",dscom,dsfecha,idactivo";
		$sql.=") values (";
		$sql.="$idproducto,'$dsnombre','$dsapellidos',";
		$sql.="'$dsemail','$dsempresa','$dscargo','$dspais','$dsciudad','$dstelefono','$dsmovil','$dsdireccion',";
		$sql.="'$dscom','".date("d/m/Y")."',1)";		
		//echo $sql;				
		//exit;
		$db->Execute($sql);//exit();
		$redir="../../gracias.solicitar.php?idg=3";
} else { 
	$redir="../../gracias.solicitar.php?idg=3";
}		
