...
$titulomodulo="Configuracion de Estados  de las Compras";
$dsm=$_REQUEST['dsm'];
...
if ($dsm<>"" && $idpos<>"") { 
		$sql="select id ";
		 ...
		 if (!$result->EOF) {
			...
			$sql.=" values ('$dsm',$idpos,$idactivo,'".limpieza(strtolower($dsm))."') ";
			if ($db->Execute($sql))  { 
				$error=0;
				...
				$dsruta="../ecommerce/estadocompras/default.php";
...
// modificacion rapida
		...
		$h=0;
			for ($j=0;$j<$contarx;$j++){
				if ($_REQUEST['id_'][$j]<>"" && $_REQUEST['dsm_'][$j]<>""){
