...
$idmostrar=$_REQUEST['idmostrar'];

if ($dsalias=="") $dsalias=$dsm;
$dsmayorista=$_REQUEST['dsmayorista'];
...
if ($dsm<>"" && $idpos<>"") {
		$sql="select id ";
	 	$sql.=" from $tabla WHERE dsm='$dsm' and idtipo=$idtipo and idnat=$idnat and dsmayorista='$dsmayorista'";
		 ...
		 if (!$result->EOF) {
			...
			$sql="insert into $tabla (dsm,idpos,idactivo,idnat,idtipo,dsalias,dsmayorista,dsruta,idmostrar)";
			$sql.=" values ('$dsm',$idpos,$idactivo,$idnat,$idtipo,'$dsalias','$dsmayorista','".limpieza(strtolower($dsalias))."',$idmostrar) ";
			if ($db->Execute($sql))  {
				$error=0;
		...
		$h=0;
			for ($j=0;$j<$contarx;$j++){
				if ($_REQUEST['id_'][$j]<>""){
					...
					$sql.= ",dsruta='".limpieza(strtolower($_REQUEST['dsalias_'][$j]))."'";
					$sql.= ",idpos=".$_REQUEST['idpos_'][$j]."";
					$sql.= ",idnat=".$_REQUEST['idnat_'][$j]."";
					$sql.= ",idmostrar=".$_REQUEST['idmostrar_'][$j]."";

					$sql.= ",dsalias='".$_REQUEST['dsalias_'][$j]."'";
					$sql.= ",dsmayorista='".$_REQUEST['dsmayorista_'][$j]."'";
...
$sql="select a.id,a.dsm,a.idpos,a.idactivo,a.dsimg1,a.idnat,a.dsalias,a.dsmayorista,a.idmostrar from $tabla a where id>0 and idactivo<>9 and idtipo=$idtipo";
