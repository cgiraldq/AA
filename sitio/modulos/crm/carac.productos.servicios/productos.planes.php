...
//$db->debug=true;
$tabla="tblproductosxplanes";
$titulomodulo="Configuracion de planes";
...
if ($v==1){
	...
	$strSQL="select id as id,dsz dsm,idactivo from $tabla where 1 and idactivo not in (9) ";
...
?>
	<form action="<? echo $pagina;?>" method="post" name=xx1>
		...
		?>
					...
					<td><input style="width: 457px;text-align: center;" type="text" class=text1 value="<? echo $fila->dsm?>" name="dsm[]"></td>
					<td>
					...
					<td>
					<?
						$rutax="productos.modplanes.php?tabla=".$tabla."&dir=".$dir."&idcampo=".$fila->id ;
