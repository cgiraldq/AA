		...
		if ($num>=1){
			...
			if (mysql_db_query($dbase,$strSQL,$db)) {
				...
				if (mysql_num_rows($vermasx)>0) {
					...
					<!--
					location.href="productos.catplanes.php?tabla=<? echo $tabla;?>&dir=<? echo $dir;?>&idcampo=<? echo $idcampo;?>";
					//-->
					</script>
					<?
				}
...
<? include ("../../incluidos/resultoperaciones.php"); ?>
		...
		<tr bgcolor="<? echo $fondos[12];?>" align=center>
			...
			<input type=button name=enviar value="Regresar" class=formabot onClick="irAPaginaD('productos.catplanes.php');">
