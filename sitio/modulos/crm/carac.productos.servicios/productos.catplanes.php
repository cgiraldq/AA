...
$titulomodulo="Configuracion de categoria de plan";

//insercion de la categoria del planes
if ($dsm<>"" && $idpos<>"") {
		$sql="select id ";
		 ...
		 if (!$result->EOF) {
			...
			$error=1;
		 	...
		 	// insertar
			$sql="insert into $tabla (dsz,idpos,idactivo,idempresa)";
			...
			$sqld="select id,dsz from $tabla where dsz='".$dsm."'";
...
if ($v==1){
	...
	$strSQL="select id as id,dsz as dsm,idactivo from $tabla where 1 and idactivo not in (9) ";
...
?>
	<form action="<? echo $pagina;?>" method="post" name=xx1>
		<table width=100% align=center  cellpadding=0 class="text1">
			<tr class="tbl_cbz_lista">
		...
		if (mysql_num_rows($vermas)>0){
			...
			$colorlineal ="#f3f3f3";
		?>
					<tr class=textnegro  bgcolor="<?if($contar%2==0){echo $colorlineal;}else {	$colorlineal= '#FFF' ;echo $colorlineal; }?>" align="center" onmouseover="mOvr(this,'#CCCCCC');" onmouseout="mOut(this,'<? echo $colorlineal; ?>')">
					<td><input type="text" class=textnegro value="<? echo $fila->dsm?>" name="dsm[]"></td>
					<td>
				...
				<option value="2" <? if ($fila->idactivo=="2"){ echo "SELECTED";}?>>NO</option>
					...
					<td>
					<?
						$rutax="productos.modcatplanes.php?tabla=".$tabla."&dir=".$dir."&idcampo=".$fila->id."&titulomodulo=".$titulomodulo;
