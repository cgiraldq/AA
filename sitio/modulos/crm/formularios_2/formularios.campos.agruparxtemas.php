...
$titulomodulo="Agrupamiento de campos por temas ( $dstema )";
...
$tabla="framecf_tbltiposformulariosxcamposxagrupamientoxtemasxagrupados";
// insercion
if ($dsm<>"" && $idpos<>"") {
		$sql="select id ";
	 	$sql.=" from $tabla WHERE dsm='$dsm' and idagrupamiento='$idx' ";
		 ...
		 if (!$result->EOF) {
			...
			$error=0;
		 	...
		 	// insertar
			$sql="insert into $tabla (dsm,idpos,idactivo,idformulario,idagrupamiento)";
			$sql.=" values ('$dsm',$idpos,$idactivo,$idxx,$idx) ";
			//echo $sql;
			if ($db->Execute($sql))  {
...
// eliminacion
include($rutxx."../../incluidos_modulos/modulos.papelera.temas.php");
...
<body>
	...
	<? include($rutxx."../../incluidos_modulos/navegador.principal.php");
	 ...
	 $campoletra="dsm";
		...
		$dsrutap="../crm/formularios/formularios.campos.agruparxtemas.php";
...
$sql="select a.id,a.dsm,a.idformulario,a.idpos,a.idactivo,a.idtienda,a.dsmalterna,idsitemap from $tabla a where idformulario='$idxx' and idactivo not in(9) and idagrupamiento=$idx ";
...
$result=$db->PageExecute($sql,$maxregistros,$pagina_actual);
	if (!$result->EOF) {
		...
		include("tabla.agruparxtemas.php");
