...
<table width="70%" border="0" cellpadding="0" cellspacing="0" class="texto_centro" >
        <tr>
         	<td width="615" align="left" valign="middle">
                ...
                <h1>Registro del formulario <? echo $nombreform;?>
<? if ($exportardatos<>1){?>
<input type="button" class="botones" value="Ingresar registro"onclick="irAPaginaD('formularios.vistaprevia.php?idx=<? echo $idxx;?>&idgaleria=<? echo $idgaleria;?>')">
<?}?>
                </h1>
...
<?
 ...
 if($_REQUEST['idxx']==1)$user=",Usuario/Asesor";
 		...
 		if($idxx==1){
			...
			if($cont_campos<>""){
					...
					$resultxy=$db->Execute($sqlxy);
						...
						echo "<span style='color:red;cursor:pointer' title='Faltan campos obligatorios'>*</span>";
					}
				$resultxy->Close();
				...
				if(!$resultimg->EOF){
					...
					if(!$resultcant->EOF){
						 ...
						 if($totalimagenes<$cantimg){
						 	echo "<span style='color:red;cursor:pointer;font-size:14' title='Falta agregar Imagenes, minimo ".$cantimg."'> ! </span>";
						 }
					}
				$resultcant->Close();
...
<?
if($_REQUEST['idxx']==104 || $_REQUEST['idxx']==1){
...
<? if($idxx==1){?>
<a class="botones2" href="javascript:irAPaginaDN('formulario.ver.cliente.php?idy=<? echo $result->fields[3]; ?>')" type="imprimir">Ver</a>
<?}?>
...
<?}?>
			...
			<?
			  ...
			  <?
			  $rutaeditar="formularios.documentos.zona.php";
			  $rutax=$rutaeditar."?idxx=".$idxx."&idy=".$result->fields[3]."&idgaleria=".$_REQUEST["idgaleria"];
			  $formax="";
			  $mrutax="Documentos";
			  $clase="botones2";
			  include($rutxx."../../incluidos_modulos/enlace.generico.php");
			  ...
			  <?
			  $rutaeditar="registros.editar.cercanos.php";
			  $rutax=$rutaeditar."?idxx=".$idxx."&idy=".$result->fields[3]."&idgaleria=".$_REQUEST["idgaleria"];
			  $formax="";
			  $mrutax="Puntos cercanos";
			  $clase="botones2";
			  include($rutxx."../../incluidos_modulos/enlace.generico.php");
			  }
