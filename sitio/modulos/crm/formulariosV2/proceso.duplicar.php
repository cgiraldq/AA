...
// se insertan los campos duplicados pero se cambia el nombre
			...
			$sqly.=" values ('$dsm','$idactivo','$idpos','$dstablax','$dsasunto','$dsasuntoar','$dsenc','$dsremate','$dscorreo1','$dscorreo2','$dscorreo3','$idtipo','$iddes','$dstiempo',2,'$idgaleria','$idestilo' ) ";

			$resulty=$db->Execute($sqly);
			$resulty->close();
			 $id=mysql_insert_id();

		$sqlx="CREATE TABLE crm_".strtolower(limpiezatablas(utf8_decode($dsm)))." LIKE crm_".strtolower(limpiezatablas(utf8_decode($dsmx))).";";
		//echo $sqlx;
		//exit();
		$db->Execute($sqlx);
		 }
