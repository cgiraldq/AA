...
<form action="<? echo $pagina;?>?idc=<? echo $idc;?>" method=post name=p>
<?
		...
		?>
		 <tr bgcolor="<? echo $fondo?>" onMouseOut="mOut(this,'<? echo $fondo;?>');" onMouseOver="mOvr(this,'<? echo $fondo3;?>');" >
		  ...
		  <select name="idactivo_[]" class="textnegro">
		  	...
		  	<option value="3" <? if ($result->fields[3]==3) echo "selected";?>>DESTACADO HOME</option>
		  	<option value="4" <? if ($result->fields[3]==4) echo "selected";?>>DETALLE</option>
		  </select>
			</td>
		  ...
		  <?
		  $rutax="hoteles.galeria.editar.php?idx=".$result->fields[0]."&idc=".$result->fields[4];
		  include($rutxx."../../incluidos_modulos/enlace.detalles.php");?>
		  |
		  <?
		  $rutax=$pagina."?idx=".$result->fields[0]."&idc=".$idc;
		  $formax="";
...
<input type="button" name="regresar" value="Regresar" class="botones" onclick="irAPaginaD('../formularios/registros.php?idxx=54&r=1')">
