...
function preferencias($idpreferencia,$tabla,$tabla1,$idcampo,$cant){
	global $db;
		$sql="Select a.id,a.dsz ";
		$sql.=" from $tabla a where  ";
		$sql.=" a.idactivo not in(2,9) and a.preferencia_asociada=$idpreferencia order by a.dsz asc ";
...
?>
	...
	<form action="<? echo $pagina;?>" method="post" name=xx1>
		...
		$sql="Select a.id,a.dsz ";
		$sql.=" from $tabla a where  ";
		$sql.=" a.idactivo not in (2,9) and a.preferencia_asociada=0 order by a.dsz asc ";
		//echo $sql;
		 $result=$db->Execute($sql);
		 if (!$result->EOF) {
		?>
			...
			<?
			while (!$result->EOF) {
			include("producto.categorias.datos.php");
			$result->MoveNext();
			 if (!$result->EOF) {
				include("producto.categorias.datos.php");//2
				$result->MoveNext();
			 }
			 if (!$result->EOF) {
				include("producto.categorias.datos.php");//2
...
<?
		...
		<input type=hidden name=idy value="<? echo $_REQUEST['idcampo'];?>">
		<input type=hidden name=dstabla value="<? echo $_REQUEST['tabla'];?>">
