...
if ($dir==""){
	...
	$idcampo=$_REQUEST['idcampo']; // nombre
...
if ($dir==1){
	$mensajeData="Ingresando galeria para producto o servicio seleccionado";
	// armando vector de campos
	$camposN[0]="Nombre";
	$camposN[2]="Activo?";

	if ($_REQUEST['inn']==1){
		$strSQL=" select dsz from ".$tabla." where dsz='$campo0' and idproducto=$idcampo and idempresa=".$_SESSION['i_idempresa'];
		$vermas=mysql_db_query($dbase,$strSQL,$db);
		$num=mysql_num_rows($vermas);
		if ($num>=1){
			$val=1;
		} else {
			$strSQL="insert into ".$tabla." (idempresa,dsz,idactivo,idproducto)";
			$strSQL.="  values (";
			//$strSQL.=" '',".$_SESSION['i_idempresa'].",$campo3,'$campo0','$campo1',";
			$strSQL.=" ".$_SESSION['i_idempresa'].",'$campo0'";

			$strSQL.=",$campo2,$idcampo";
			$strSQL.=" )";
			if (mysql_db_query($dbase,$strSQL,$db)) {
				$val=2;
				$strSQL=" select idz from ".$tabla." where dsz='$campo0' and idproducto=$idcampo and idempresa=".$_SESSION['i_idempresa'];
				$vermasx=mysql_db_query($dbase,$strSQL,$db);
				if (mysql_num_rows($vermasx)>0) {
					$idcampox=mysql_result($vermasx,"0","idz");
					?>
					<script language="javascript">
					<!--
					location.href="galeria.mod.php?tabla=<? echo $tabla;?>&dir=<? echo $dir;?>&idcampox=<? echo $idcampox;?>&idcampo=<? echo $idcampo;?>";
					//-->
					</script>
					<?
				}
...
<? include ("../../incluidos/resultoperaciones.php"); ?>	
		...
		<td valign=top>
			...
			<option value="3" <? if ($_REQUEST['campo2']=="3"){ echo "SELECTED";}?>>Destacada</option>
			<option value="4" <? if ($_REQUEST['campo2']=="4"){ echo "SELECTED";}?>>Detalle</option>
		...
		<tr bgcolor="<? echo $fondos[12];?>" align=center>
			...
			<input type=button name=enviar value="Regresar" class=formabot onClick="irAPaginaD('galeria.php?idcampo=<? echo $idcampo?>');">
