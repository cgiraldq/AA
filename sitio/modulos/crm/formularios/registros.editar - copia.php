...
$rc4 = new rc4crypt();
$apagar=1;
$titulomodulo="Listado de registros formularios.";
...
$rr="registros.php?idxx=".$idx."&idgaleria=$idgaleria";
...
$rutamodulo.="  <a href='registros.php?idgaleria=$idgaleria&idxx=$idx' class='textlink'>$titulomodulo</a>  /  <span class='text1'>Modificar</span>";
...
if($agrupamiento=="" && $idx==104) $sqlx.=" and a.idoblig=1";

if($agrupamiento<>""){
		$sqlx.=" order by b.idpos asc";
} else{
	$sqlx.=" order by a.idpos asc";
}
//echo $sqlx;
...
<table>
	...
	<td colspan="2">
		...
		<input type=button name=enviar id="detalle" value="Detalle"  class="botones" onClick="Abrir_capa('detalle')">
		<input type=button name=enviar id="galeria" value="Galeria"  class="botones" onClick="Abrir_capa('galeria')">
		<input type=button name=enviar id="observaciones" value="Observaciones"  class="botones" onClick="Abrir_capa('observaciones')">
...
?>
		...
		<?if($_SESSION['i_idperfil']==1){?>
			<select name="idusuario" style="width:100%" id="idusuario">
...
<?
   			while(!$result->EOF) {
   				...
   				$dsm=$result->fields[1];
?>
			<?if($_SESSION['i_idperfil']==1){?><option value="<? echo $idusuario;?>" <? if($iduser==$idusuario)echo "selected='selected'";?> ><? echo $dsm;?></option><?}?>
...
<td align="center" style="font-size:12">Usuario asociado</td>
<td>
<input type="text" readonly="readonly" value="Sin seleccionar">
...
if ($idoblig==1) {
	...
	if($idtipo==3){ // input tipo password
		...
		if($dsmcampo==$campoe){
			...
			$valorcampo1="$valorcampoe";
			$valorcampo=$rc4->decrypt($s3m1ll4, urldecode($valorcampoe));
			//$valorcampo="$valorcampoe";
		...
		$dscontrasena=$rc4->decrypt($s3m1ll4, urldecode($valorcampo1));
		$tam=45;
		$mensaje_capa="$dsmensaje";
		$tipocampo=6;
		$descripcion=$dsdes;
...
<input type="hidden" name="idagrupamiento"  value="<? if($_REQUEST['agrupamiento']==""){echo $id_agrupamiento;}else{echo $_REQUEST['agrupamiento'];};?>">
<tr>
...
<?
if($_SESSION['i_idperfil']==1 || $_SESSION['i_idusuario']==$idusuario){
...
<tr id="galeria1" style="display:none" >
	<td><? include('formulario.galeria.php'); ?></td>
</tr>

<tr id="observaciones1" style="display:none" >
...
function Abrir_capa(parametro){
	...
	if(parametro=="detalle"){
  		...
  		document.getElementById('observaciones1').style.display='none';
  	}
