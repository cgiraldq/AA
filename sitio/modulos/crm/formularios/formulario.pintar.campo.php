...
$dsmform=seldato("dstabla","id","framecf_tbltiposformularios",$_REQUEST['idx'],2);
$sqle.=" from $dsmform a where id='".$_REQUEST['idy']."' ";
//echo $sqle;
//echo "<br>";
...
if($interno=="")	$param.=$dsmcampo.",";
	...
	if($idtipo==15){ // campo tipo seleccionador externo
		$valores="";
		// consulta para listar los option del selecionador
		$sqlz="select a.id,a.dsm,a.dsvalor from framecf_tbltiposformulariosxcampos a ";
		$sqlz.="where  a.idcampo=$id  ORDER BY a.idpos ASC ";
		//echo $sqlz;
		$resultz=$db->Execute($sqlz);
        if(!$resultz->EOF){
      		$idf=$resultz->fields[0];
        	$dsmf=$resultz->fields[1];
        	$valor=$resultz->fields[2];
        	 $campo="$dsmcampo";
        	 $titulocampo="$dsm"." $oblig ";

        	$dsmform=seldato("dstabla","id","framecf_tbltiposformularios",$dsmf,2);
        	$sqlf=" SELECT id,$valor FROM $dsmform WHERE idactivo=1";
        	//echo $sqlf;
		...
		$cont=1;
		 ...
		 while(!$resultf->EOF){
		 		$idselect=$resultf->fields[0];
		        $dsmx=$resultf->fields[1];
		        // aqui se pintan los valores de los option
				 $valores.="$idselect|$dsmx;";
				$resultf->MoveNext();
				$cont=$cont+1;
...
if($idtipo==16){ // input tipo archivo
		$campo="$dsmcampo";
		$contadorx="counter_$campo";
		$tam=60;
		$valorx="255";
		...
		$descripcion=$dsdes;
		$tipoarchivo=1;
		$borrararchivo="borrar_".$dsmcampo;
		$carchivoanterior="carchivoanterior";
		$narchivoanterior="narchivoanterior_".$dsmcampo;
		$rutaImagen="../../../../contenidos/images/archivosformularios/";
		include($rutxx."../../incluidos_modulos/control.texto.form.php");
		$nombre_capa="capa_$dsmcampo";
		$mensaje_capa="Debe digitar la $dsmcampo";
	}
...
<td><? echo $dsm;?></td>
<td>
<select  onchange="validarbarrio2('<? echo $dsmcampo;?>','<? echo $resultx->fields[0]?>','dscampo27','','','../../../')"  name="<? echo $dsmcampo;?>" id="<? echo $dsmcampo;?>"   >
<option value=""> --- Seleccionar --- </option>
 </select>
 <?
$tipocampo=12;
