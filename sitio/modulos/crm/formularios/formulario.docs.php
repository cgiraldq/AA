...
//$db->debug=true;
$tablaasoc="crm_documentos_por_clientes";
$idy=$_REQUEST['idy'];
...
function preferencias($idpreferencia,$tabla,$tabla1,$idcampo,$cant){
	global $db;
		...
		$sql.=" a.idestado=1 and a.preferencia_asociada=$idpreferencia order by a.titulo asc ";
		...
		if (!$resulty->EOF) {
			...
			$nombrepreferencia=$resulty->fields[1];
				...
				$sql.=" and documento_asociado=".$idpreferencia;
...
if ($_REQUEST['idasoc']<>""){
			...
			for ($j=0;$j<$contar;$j++){
					...
					if ($_REQUEST['idasoc'][$j]<>"") {
						...
						$sqlm=" insert into $tablaasoc   (cliente_asociado,documento_asociado) ";
						$sqlm.= " values (";
						$sqlm.= $idy.",";
...
?>
<table width="100%" border="0" class="campos_ingreso">
<?
...
?>
	...
	<table width=100% align=center cellspacing=0 class="campos_ingreso">
		...
		$sql="Select a.id,a.titulo ";
		$sql.=" from $tabla a where  ";
		$sql.=" a.idactivo=1 and a.preferencia_asociada=0 order by a.titulo asc ";
		 $result=$db->Execute($sql);
		 if (!$result->EOF) {
		?>
		...
		<tr valign=top>
			<?
			while (!$result->EOF) {
			include("formulario.docs.datos.php");
			$result->MoveNext();
			 if (!$result->EOF) {
				include("formulario.docs.datos.php");//2
				$result->MoveNext();
			 }
			 if (!$result->EOF) {
				include("formulario.docs.datos.php");//2
