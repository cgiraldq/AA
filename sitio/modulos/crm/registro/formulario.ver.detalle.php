...
?>
      ...
      <img src="<? echo $rutaImagend2.$dsimg1empresa ?>" >
		...
		<td align="center">
			<strong>INFORMACI&Oacute;N REGISTRO ASOCIADO</strong>
		</td>
	</tr>
</table>
...
$sqlx="select b.id,b.dsm,b.dscampo,b.idtipo from $tablax b where id>0 and idtipoformulario='$tipo' ";
$sqlx.="and idactivo=1";
$sqlx.=" order by idpos";
//echo $sqlx;
if ($tipo==110) {
		$idconsecutivo=seldato("idconsecutivo","id"," crm_propiedades",$_REQUEST['id'],1);
		$idconsecutivo=str_pad($idconsecutivo,4,"0", STR_PAD_LEFT);
}

$result_campos=$db->Execute($sqlx);
if (!$result_campos->EOF) {
?>
<table width="100%" border="0" cellpadding="4" cellspacing="0" class="text1">
<?
	while(!$result_campos->EOF){
		...
		$sql=" SELECT $dscampo FROM $tabla WHERE $dscampo !=''";
		if ($tipo==110) {

			$sql.=" and codigo_propiedad='$idconsecutivo' ";
...
?>
<tr class="tbl_cbz_core" valign="top" align="center" onMouseOut="mOut(this,'<? echo $fondo;?>');" onMouseOver="mOvr(this,'#E8E8E8');">
	<td align="left"><strong><? echo $dsm;?></strong></td>
	<td align="left"> <? echo reemplazar($valorbase);?>
	</td>
</tr>
<?
		}
