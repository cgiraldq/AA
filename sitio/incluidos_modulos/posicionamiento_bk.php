...
if(($dsnombre<>"" || $idrelacion<>"" || $_REQUEST['id']<>"") &&  $pag=="blog.php"){
	$sql="select dsm,dsd2,dsruta,dsimg,id from  blogtblblog where (dsruta='$dsnombre' ";
	if ($idrelacion<>"") $sql.=" or id=".$idrelacion;
	...
	if (!$result->EOF) {
		...
		$rutaimagen_sindicacion = $rutax_sindicacion.'contenidos/images/blog/'.trim($result->fields[3]);
