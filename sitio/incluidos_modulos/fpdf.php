...
function Output($name='',$dest='')
{
	...
	switch($dest)
	{
		...
		case 'F':
			//Save to local file
			$ruta="../../contenidos/images/certificados/".$name;
			$f=fopen($ruta,'wb');
			if(!$f)
...
function ImprovedTable($header,$data)
{
	...
	*/
    ...
    foreach($data as $row)
    {
        $this->Cell($w[0],6,$row[0],0,0,'LR');
        $this->Cell($w[1],6,$row[1],0,0,'LR');
		/*
...
//Tabla simple
function BasicTable_($header,$data)
{
    //Cabecera
    foreach($header as $col)
        $this->Cell(40,7,$col,1);
