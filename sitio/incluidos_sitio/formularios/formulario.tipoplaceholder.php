...
//if($idformulario==104)$sqlx.=" and a.dscampo!='dscampo2'";
...
if ($idoblig==1) $param.=$dsmcampo.",";
	...
	<fieldset>
			...
			<div><input  placeholder="<? echo reemplazar($dsm); if ($idoblig==1){ echo "*";}?>" type="text" <? if($dsmcampo=='dscampo3')echo 'maxlength=11';?>  name="<? echo $dsmcampo;?>" id="<? echo $dsmcampo;?>"  onkeypress="valFormulario('<?echo $forma?>','<? echo $param?>',1,'capax_<? echo $dsmcampo;?>','')"></div>
  ...
  <fieldset>
  		...
  		<div>
  			<select name="idagrupamiento" id="idagrupamiento" style="width:100%">
  				<option value="0"> Seleccionar Tipo<?if ($idoblig==1){ echo "*";}?></option>
  				<? while (!$result->EOF) {
				$id=$result->fields[0];
				$dsm=$result->fields[1];
			?>
