    <article id="actualizar_datos_form" class="cuerpo_tab">
        ...
        <?
            $sqlac="select dsnombres,dsapellidos,dstipoidentificacion,dsidentificacion,dscorreocliente,dstelefono,dstelefono2,dsmovil,dsfax,dsdireccion,dsempresa,dscargo,dsciudad,dsdepartamento,dsfacebook,dstwitter,dsfechanacimiento,id,dsacepto,dspais from tblclientes where id=".$_SESSION['i_idcliente'];
            //echo $sqlac;
        ...
        <form action="modulos/validaciones/actualizar.zona.php" name="frm_actualizar_zona" method="post"  autocomplete="off" id="frm_actualizar_zona">
            ...
            <fieldset></fieldset>
            <fieldset></fieldset>
            <fieldset class="campo_dirección">
                <label for="dsdireccion">Direcci&oacute;n exacta de entrega *</label>
...
<!--             <fieldset class="cont_boletin">
    ...
    <input type="checkbox" name="dsacepto" id="dsacepto" <?if ($dsacepto==1) echo "selected"?>> <label for="dsacepto">Desea recibir nuestro boletín</label>
            ...
            <fieldset class="cont_btns">
            <nav class="btn_derecha">
                <input type="hidden" value="<?echo $id?>" id="idx" name="idx">
                <input type="button" value="Modificar" class="btn_buscador" onclick="valFormulario('<?echo $forma?>','<? echo $param?>',0,'','<? echo $rut ?>');">
            </nav>
            </fieldset>
        </form>
        <?
