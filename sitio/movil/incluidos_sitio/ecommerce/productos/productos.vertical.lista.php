...
<article class="cont_noticas_centro_vertical">
	...
	<article id="lista_prodcutos">

<?
$rutaImagen=$rutaFuenteImagenes."/contenidos/images/productos/";

            $sql="select a.id,a.dsm,a.dsruta,a.dsimg1,a.precio1,a.iva,a.preciodescuento,a.dsd,a.idactivo,a.idnat,a.preciocompra,a.precio2";
...
if ($valorseguro<0) $valorseguro=0;
		...
		<article class="noticas_centro_vertical">
			...
			<? if ($dsimg1<>""){?>
				<a href="<? echo $dsruta?>">
					<img src="<? echo $rutaImagen.$dsimg1?>" alt=""> <!-- tama&ntilde;o de la Imagen 189 x 189 -->
				</a>
			<?}?>
			<section class="info_prod">
				<ul class="btns_tienda">
					<!-- <li><a href="adicionar.php?idproducto=<? echo $id?>" class="btn_tienda">COMPRAR</a></li> -->
                    <li><a href="<? echo $dsruta?>" class="btn_tienda">COMPRAR</a></li>
				</ul>

			<h3><? echo $dsm?></h3>
			<p><? //echo $dsd?></p>
			     <? if ($id==416 || $id==414 || $id==415) {?>
            <p class="precio1">$ <? echo number_format($preciodescuentom,0) ?></p>
            <? if ($preciodescuento>0) {?><p class="precio2">$ <? echo number_format($precio1m,0) ?></p><? } ?>

        <? } else {?>
            <p class="precio1">$ <? echo number_format($preciomostrar+$precio2+$valorseguro+$iva,0) ?></p>
