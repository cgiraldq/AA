...
<article id='comentar' class="cont_frm_horizontal  cuerpo_tab" style="display:<? echo $cap ?>;">
	...
	<form action="<?echo $rutbase?>/modulos/validaciones/comentariob.php" name="frm_comentarx" method="post" id="frm_comentarx">
		...
		<nav class="btn_derecha">
			<input type="button" <? if($_SESSION['idioma']==1){?>value="Comentar"<?}?> <? if($_SESSION['idioma']==2){?>value="comment"<?}?> class="btn_general" onclick="valFormulario('<?echo $forma?>','<? echo $param?>',0,'','<? echo $rutalocal."/"; ?>');">
			<input type="reset" <? if($_SESSION['idioma']==1){?>value="Cancelar"<?}?> <? if($_SESSION['idioma']==2){?>value="Cancel"<?}?> class="btn_general">
