...
class Magestore_Bannerslider_Adminhtml_Bannerslider_BannerController extends Mage_Adminhtml_Controller_Action {
    ...
    public function saveAction() {        
        if ($data = $this->getRequest()->getPost()) {          
            ...
            try {
								$datestr = str_replace("/","-",$data['start_time']);
								$dateend = str_replace("/","-",$data['end_time']);
                 $data['start_time']=date('Y-m-d H:i:s',Mage::getModel('core/date')->gmtTimestamp(strtotime($datestr)));
                 $data['end_time']=date('Y-m-d H:i:s',Mage::getModel('core/date')->gmtTimestamp(strtotime($dateend)));
            } catch (Exception $e) {}
