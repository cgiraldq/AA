jQuery(document).ready(function() {
  ...
  /*document.querySelector(".powr-video-slider iframe").load(function() {
    document.querySelector("#appView .powrMark").display = "none";
  });*/
  ...
  $j(".blog-post-view .breadcrumbs li.blog").html(
    "<a href='http://www.adrianaarango.com/store/prensa'>VOLVER</a>"
  );
  $j(".cms-pagina-postead .breadcrumbs .cms_page").html(
    "<a href='http://www.adrianaarango.com/store/prensa'>VOLVER</a>"
  );
  ...
  if (ancho < 776) {
    ...
    jQuery(".catalog-category-view .main").animate(
      { marginTop: altoimagen },
      0
    );
  ...
  jQuery(window).on("scroll", function() {
    if ($j(window).width() > 468) {
      if (jQuery(window).scrollTop() > 470) {
        jQuery(".cms-home .page-header-container").removeClass(
          "menu-fixed-old"
        );
        jQuery(".cms-home .page-header-container").addClass("menu-fixed");
      } else {
...
function instagram() {
  jQuery("#instansive_e6ac80e19f").height(
    jQuery("#instansive_e6ac80e19f").width() * 2
  );
}
...
function modifyHeightSkipCart() {
  ...
  jQuery(".skip-cart").click(function() {
    if (jQuery(this).hasClass("skip-active")) {
      ...
      var height_div =
        jQuery("#header-cart").outerHeight() +
        jQuery(".page-header-container").outerHeight();
      var height_window = jQuery(window).height();
      var new_height_div = height_window;

      if (height_div > height_window) {
        jQuery("#header-cart").css({
          height: new_height_div,
          "overflow-y": "auto",
          "overflow-x": "hidden",
