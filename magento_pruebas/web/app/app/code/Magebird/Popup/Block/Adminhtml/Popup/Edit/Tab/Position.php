    ...
    \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        $note = "<p class='nm'><small>".__("If you selected percentage for 'Popup width unit' you can only use Center.")."</small></p>";              
        ...
        $fieldset->addField('vertical_position','select',array(
                    ...
                    'required'=>true, 
              		  'values'    => array(
              			  array(
              				  'value'     => 1,
              				  'label'     => __('Define px from top'),
              			  ),            
              			  array(
              				  'value'     => 2,
              				  'label'     => __('Define px from bottom'),
              			  ),     
              			  array(
              				  'value'     => 3,
              				  'label'     => __('Top px absolute to defined element'),
              			  ),  
              			  array(
              				  'value'     => 4,
              				  'label'     => __('Show on top and push page content down'),
              			  ),                                                                                                                                       
              		  ),            
        ));
        
        $fieldset->addField('vertical_position_px','text',array(
                    'label' => __('How many px'),
                    'name' =>  'vertical_position_px',
                    'required'=>true,                                                                                                
        ));   
        
        $note = '<p class="nm"><small>' .  __("If you selected 'Left px absolute to defined element' and 'Top px absolute to defined element', you need to define from which element should be calculated left position. Write element selector. Otherwise leave empty. e.g. #idName, .className, div input#idName. Read more about selectors <a href='http://www.w3schools.com/jquery/jquery_ref_selectors.asp' target='_blank'>here</a>") . '</small></p>';
