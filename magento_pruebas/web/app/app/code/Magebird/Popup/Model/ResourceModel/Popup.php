...
class Popup extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {          
      ...
      function mailchimpVars($content){                
          ...
          require_once($this->dir->getPath('lib_internal') . '/magebird/popup/MCAPI.class.php');
          $api = new MCAPI($this->scopeConfig->getValue('magebird_popup/services/mailchimp_key'));            
          $mailchimpListId = explode('"',$explode[1]);
          $mailchimpListId = $mailchimpListId[0];          
          if($mailchimpListId){
            $results = $api->listMergeVars($mailchimpListId);
            $tagExists = false;
            if(is_array($results)){
            ...
            if(!$tagExists){
              $api->listMergeVarAdd($mailchimpListId,'POPUP_COUP','Popup Coupon Code');
            }
            
            if($api->errorCode){
              $this->messageManager->addError("Mailchimp api error ".$api->errorMessage);
            }         
          }              
      }
